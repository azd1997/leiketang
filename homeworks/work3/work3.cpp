...
zfill(int n, int width) {
    ...
    string left_zeros = nChar(width - len);
    string res = left_zeros + n_str;
    return res;
}
...
//
// 先修改nChar函数（重载）
string nChar(int n, char fillchar='0') {
    string r = "";
    int i = 0;
    while(i < n) {
        r += fillchar;
...
rjust(const string &s, int width, char fillchar=' ') {
    ...
    return nChar(width - s_len, fillchar) + s;
}
...
string ljust(string s, int width, char fillchar=' ') {
    ...
    int s_len = s.length();
    if (s_len >= width) return s;
    return s + nChar(width - s_len, fillchar);
}
...
center(const string &s, int width, char fillchar=' ') {
    ...
    int delta = width - s_len;
    if (delta % 2 == 0) {   // 两边可以补相同数量的字符
        return nChar(delta / 2, fillchar) + s + nChar(delta / 2, fillchar);
    } else {    // 左边补少一个
        return nChar(delta / 2, fillchar) + s + nChar(delta / 2 + 1, fillchar);
    }
}
...
isSpace(const string &s) {
    ...
    if (s.length() == 0) return false;  // 空字符串不是空格字符串
    for (char c : s) {
        if (c != ' ') return false;
    }
    return true;
...
void
testIsSpace() {
    ensure(isSpace(" "), "isSpace test 1");
    ensure(isSpace("   "), "isSpace test 2");
    // C++ 中 not 可以替代 !
    // 同理还有 and 替代 && 和 or 替代 ||
    ensure(not isSpace(""), "isSpace test 3");
    ensure(not isSpace("gua"), "isSpace test 4");
    ensure(not isSpace("  gua"), "isSpace test 5");
}
...
stripLeft(const string &s) {
    ...
    int idx = 0, s_len = s.length();
    for (; idx < s_len; idx++) {
        if (s[idx] != ' ') break;
    }
    if (idx == s_len) return "";
    return s.substr(idx, s_len - idx);
}
...
stripRight(const string &s) {
    ...
    int s_len = s.length(), idx = s_len - 1;
    for (; idx >= 0; idx--) {
        if (s[idx] != ' ') break;
    }
    if (idx == -1) return "";
...
replace(const string &s, const string &old_str, const string &new_str) {
    ...
    // 暴力匹配
    int s_len = s.length(), idx = 0;
    int old_len = old_str.length();
    for (int i = 0; i < s_len; i++) {
        if (s.substr(i, old_len) == old_str) {
            idx = i;
            break;
        }
    }
    return s.substr(0, idx) + new_str + s.substr(idx + old_len);
}
