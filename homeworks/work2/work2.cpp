...
lowercase1(const string &s) {
    ...
    int n = s.length();
    for (int i = 0; i < n; i++) {
        if (s[i] - 'a' >= 0 && s[i] - 'a' < 26) {    // 'A-Z' ... 'a-z' // 这里说明是小写字母
            res += s[i];
        } else if (s[i] - 'A' >= 0 && s[i] - 'A' < 26) {    // 大写字母
            res += lower[s[i] - 'A'];
        } else {    // 非字母
            res += s[i];
        }
...
decode2(const string &s, int shift) {
    ...
    int idx;
    for (int i = 0; i < n; i++) {
        idx = (26 + s[i] - 'a' - shift) % 26;   // 由于直接减shift可能得到负值，所以加上26先
        res += lower[idx];
    }
    return res;
}
...
decode4() {
    ...
    // 首先将code转为小写
    string codeLower = lowercase1(code);

    string res;
    // 这里只能不断尝试1-25这些shift值，肉眼观察其是否符合意义
    for (int shift = 0; shift < 26; shift++) {
        res = decode3(codeLower, shift);
        cout << res << endl;
    }
}
...
main(int argc, const char *argv[]) {
    ...
    string s = "you are really a little girl";
    cout << encode3(s, 7) << endl;
